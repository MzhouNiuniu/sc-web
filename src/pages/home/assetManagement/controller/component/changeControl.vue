    ...
    export default {
        ...
        methods: {
            saveEdit (name) {
                ...
                this.$refs[name].validate(async valid => {
                    if (valid) {
                        ...
                        const res = await this.http.postJson(this.api.controller.changeController, params)
                        if (res.code == 200) {
            ...
            async getChangeWay () {
                ...
                if (res.code == 200) {
                    this.changeWaysOption = []
                    res.data.changeMode.forEach(ele => {
                        if (ele.sfKey !== 'sell') {
                            this.changeWaysOption.push(ele)
                        }
                    })
                }
            },
            async getLossReason () {
                ...
                if (res.code == 200) {
                    ...
                    list.forEach(element => {
                        if (element.sfValue !== '自然死亡' && element.sfValue !== '动物啃食') {
                            this.lossReasonOpt.push(element)
                        }
                    })
                }
            }
