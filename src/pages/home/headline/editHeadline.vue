<template>
    <div>
        <p class="wrapper-title"><span>书辰头条</span></p>
        ...
        methods: {
         ...
         this.seedling.content= this.$refs.editor.editorContent
               this.$refs['form'].validate( async(valid) => {
                   if (valid) {
                       ...
                       if(this.$route.query.id){
                            res = await this.http.post(this.api.headline.changeInFo,this.seedling)
                       }
                       else{

                            res = await this.http.post(this.api.headline.publish,this.seedling)
                       }
                       if(res.code==200){
                           this.$message.success('操作成功')
                           this.$router.push('/home/headline')
                       }
                   }
               })
            ...
            async getDetailsById(id){
                const res = await this.http.get(this.api.headline.getDetailsById,{id})
